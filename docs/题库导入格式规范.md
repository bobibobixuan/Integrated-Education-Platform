---
tags: [import, format, question-bank, 题库]
aliases: [导入格式, 题库格式, Import Format]
---
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| title | string | 建议填写 | 题目名称，用于教师端管理、错题统计和题库检索 |
...
3. `title` 建议写成“知识点 + 题型/场景”，不要直接复制整段题干，便于后续教师端筛选和错题回看
4. 重复导入同一文件不会去重——会新增题目
5. 单次导入最多 500 题
